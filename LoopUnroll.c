...
// Erosion with loop unrolling: processes two rows at a time (i += 2)
void erosion(){
	for(i = 1; i < M+1; i = i+2){//For each row
		for(j = 1; j < N+1; j++){//For each column
			...
			// Check for row i+1 in same loop
			if(current[i+1][j]==255 &&
			   current[i+2][j+1]==255 &&
			   current[i+1][j+1]==255 &&
			   current[i+2][j]==255){
			   output[i+1][j]=255;
...
// Dilation with loop unrolling: processes two rows at a time (i += 2)
void dilation(){
	for(i = 1; i < M+1; i = i+2){//For each row
		for(j = 1; j < N+1; j++){//For each column
